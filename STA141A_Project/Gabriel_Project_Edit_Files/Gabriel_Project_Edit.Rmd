...
**Merging Data**[Add (Hide 2011-2018)]
```{r}
...
**Change county, state to just county**[Add]
```{r}
cleandata <- cleandata%>%
...
**Percent and Dollar to integer functions**[Add]
```{r}
...
**Number of Food Insecure Persons Overall By County**[Add]
```{r}
...
**Add "County" to California counties**[Do Not add]
```{r}
california_counties <- california_counties%>%
  mutate("Name"=paste(.$Name, "County"))
```

**Add Longitude and Latitude Data**[Do Not add]
```{r}
lat <- c()
long <- c()
...
**Overall Food Insecurity per year**[Add]
```{r}
cleandata%>%
  group_by(county)%>%
...
**[NOT USED] Remaining Meal Cost After SNAP Benefits per year**[Do not add]
```{r}
cleandata%>%
  group_by(county)%>%
...
**[NOT USED] Linear Model (food insecurity ~ diff btwn snap benefits and meal cost)**[Do not add]
```{r}
g1 <- snapdata%>%
...
**Unemployment Data Cleaning**[Add]
```{r}
...
**Overall Unemployment Rate per Year**[Add]
```{r}
cleandata%>%
  group_by(county)%>%
...
**Linear Model Food Insecurity ~ Unemployment Rate**[Add]
```{r}
cleandata%>%
  mutate("year"=as.factor(year))%>%
  mutate(overall_food_insecurity_rate=per_to_num(overall_food_insecurity_rate))%>%
...
**Overall Median Income per Year**[Add]
```{r}
cleandataincome%>%
  group_by(county)%>%
...
**Linear Model Food Insecurity ~ Median Income**[Add]
```{r}
cleandataincome%>%
  mutate("year"=as.factor(year))%>%
  mutate(overall_food_insecurity_rate=per_to_num(overall_food_insecurity_rate))%>%
...
**Add Disability Data**[Add (Hidden)]
```{r}
disabilitydata <- disabilitydata%>%
  mutate(county=gsub(" County", "", .$county))%>%
  mutate(county=paste(.$county, "County"))

cleandatadisability <- inner_join(disabilitydata, cleandataincome, by = c("county", "year"))
```

**Overall Percent Disability per Year**[Do not add (use Brooke's)]
```{r}
cleandatadisability%>%
  group_by(county)%>%
  ggplot(aes(x=as.factor(year), percent_with_a_disability, fill=compare_group))+
  geom_boxplot(show.legend = F)+
  scale_fill_manual(values = c("#4E5745", "#AC5B4C"))+
  theme_minimal()+
  theme(panel.background = element_rect(fill="#E5DACF", color = "#E5DACF", size = 0.5, linetype = "solid"))+
  labs(title="Overall Disability Rate per Year",
       x="Year",
       y="Disability Rate (%)")
```

**Linear Model Food Insecurity ~ Disability Rate**[Add]
```{r}
cleandatadisability%>%
  mutate("year"=as.factor(year))%>%
  mutate(overall_food_insecurity_rate=per_to_num(overall_food_insecurity_rate))%>%
  ggplot(aes(x=percent_with_a_disability, y=overall_food_insecurity_rate))+
  geom_point(color="#4E5745")+
  geom_smooth(method="lm", color="#AC5B4C", show.legend = F)+
  theme_minimal()+
  theme(panel.background = element_rect(fill="#E5DACF", color = "#E5DACF", size = 0.5, linetype = "solid"))+
...
## Models [Add all except Classification Tree]

### Test against 2020
**[Overall] Linear Model: Food Insecurity ~ Unemployment Rate + Median Income + Disability Rate**
```{r warning=FALSE, message=FALSE}
# Define Training Data (2010-2019) and Test Data (2020)
cleandatatrain <- cleandatadisability%>%
  filter(year<2020)%>%
  select(county, overall_food_insecurity_rate, unemployment_rate_avg, median_income, percent_with_a_disability)

cleandatatest <- cleandatadisability%>%
  filter(year==2020)%>%
  select(county, overall_food_insecurity_rate, unemployment_rate_avg, median_income, percent_with_a_disability)

model <- lm(overall_food_insecurity_rate ~ unemployment_rate_avg + median_income + percent_with_a_disability, data=cleandatatrain)

lm_results <- data.frame("county"=cleandatatest$county,
           "predicted_food_insecurity_rate"=as.numeric(model$coefficients[1]) + as.numeric(model$coefficients[2])*cleandatatest$unemployment_rate_avg + as.numeric(model$coefficients[3])*cleandatatest$median_income + as.numeric(model$coefficients[4])*cleandatatest$percent_with_a_disability,
           "actual_food_insecurity_rate"=cleandatatest$overall_food_insecurity_rate)

lm_overall_acc20 <- 1 - mean((lm_results$predicted_food_insecurity_rate-lm_results$actual_food_insecurity_rate)/lm_results$actual_food_insecurity_rate)
```

**[Overall] Random Forest: Food Insecurity ~ Unemployment Rate + Median Income + Disability Rate**
```{r warning=FALSE, message=FALSE}
# Define Training Data (2010-2019) and Test Data (2020)
cleandatatrain <- cleandatadisability%>%
  filter(year<2020)%>%
...
train_rf <- train(overall_food_insecurity_rate ~ unemployment_rate_avg + median_income + percent_with_a_disability, method="rf", data=cleandatatrain, 
...
**[Overall] Classification Tree: Food Insecurity ~ Unemployment Rate + Median Income + Disability Rate**
```{r warning=FALSE, message=FALSE}
train_ct <- train(overall_food_insecurity_rate ~ unemployment_rate_avg + median_income + percent_with_a_disability, method="rpart", data=cleandatatrain, 
...
ct_overall_acc20 <- 1 - mean((ct_results$predicted_food_insecurity_rate-ct_results$actual_food_insecurity_rate)/ct_results$actual_food_insecurity_rate)
```

**Linear Model Function (lm_trainr)**
```{r}
lm_trainr20 <- function(c){
  ...
  # Check to see if Training and Test Data exists
  if (nrow(cleandatatrain)==0 | nrow(cleandatatest) == 0){
    
      lm_results <- data.frame("county"=c,
...
for (c in unique(cleandatadisability$county)){
  trainr_data <- lm_trainr20(c)
...
lm_county_acc20 <- mean(results$accuracy, na.rm=T)
```

**Random Forest Function (rf_trainr)**
```{r}
rf_trainr20 <- function(c){
  # Define Training Data (2010-2019) and Test Data (2020)
  cleandatatrain <- cleandatadisability%>%
    filter(year<2020, county==c)%>%
    select(county, overall_food_insecurity_rate, unemployment_rate_avg, median_income, percent_with_a_disability)
  ...
  # Check to see if Training and Test Data exists
  if (nrow(cleandatatrain) <= 1 | nrow(cleandatatest) == 0){
...
for (c in unique(cleandatadisability$county)){
  trainr_data <- rf_trainr20(c)
  results <- rbind(results, trainr_data[[1]])
  accuracy <- c(accuracy, trainr_data[[2]])
}

results$accuracy <- accuracy
rf_county_acc20 <- mean(results$accuracy, na.rm=T)
```

**Classification Tree (ct_trainr)**
```{r}
ct_trainr20 <- function(c){
  # Define Training Data (2010-2019) and Test Data (2020)
  cleandatatrain <- cleandatadisability%>%
    filter(year<2020, county==c)%>%
    select(county, overall_food_insecurity_rate, unemployment_rate_avg, median_income, percent_with_a_disability)
  ...
  if (nrow(cleandatatrain) <= 1 | nrow(cleandatatest) == 0){
      ...
      ct_results <- data.frame("county"=c,
             "predicted_food_insecurity_rate"=NA,
             "actual_food_insecurity_rate"=NA)
...
for (c in unique(cleandatadisability$county)){
  trainr_data <- ct_trainr20(c)
  results <- rbind(results, trainr_data[[1]])
  accuracy <- c(accuracy, trainr_data[[2]])
}

results$accuracy <- accuracy
ct_county_acc20 <- mean(results$accuracy, na.rm=T)
```
...
ct_overall_acc19 <- 1 - mean((ct_results$predicted_food_insecurity_rate-ct_results$actual_food_insecurity_rate)/ct_results$actual_food_insecurity_rate)
```

**Linear Model Function (lm_trainr)**
```{r}
lm_trainr19 <- function(c){
  
  # Define Training Data (2010-2019) and Test Data (2020)
  cleandatatrain <- cleandatadisability%>%
    filter(year<2019, county==c)%>%
    select(county, overall_food_insecurity_rate, unemployment_rate_avg, median_income, percent_with_a_disability)
...
results$accuracy <- accuracy
lm_county_acc19 <- mean(results$accuracy, na.rm=T)
```

**Random Forest Function (rf_trainr)**
```{r}
rf_trainr19 <- function(c){
  # Define Training Data (2010-2019) and Test Data (2020)
  cleandatatrain <- cleandatadisability%>%
    filter(year<2019, county==c)%>%
    select(county, overall_food_insecurity_rate, unemployment_rate_avg, median_income, percent_with_a_disability)
...
for (c in unique(cleandatadisability$county)){
  trainr_data <- rf_trainr19(c)
  results <- rbind(results, trainr_data[[1]])
  accuracy <- c(accuracy, trainr_data[[2]])
}

results$accuracy <- accuracy
rf_county_acc19 <- mean(results$accuracy, na.rm=T)
```

**Classification Tree (ct_trainr)**
```{r}
ct_trainr19 <- function(c){
  # Define Training Data (2010-2019) and Test Data (2020)
  cleandatatrain <- cleandatadisability%>%
    filter(year<2019, county==c)%>%
    select(county, overall_food_insecurity_rate, unemployment_rate_avg, median_income, percent_with_a_disability)
...
for (c in unique(cleandatadisability$county)){
  trainr_data <- ct_trainr19(c)
  results <- rbind(results, trainr_data[[1]])
  accuracy <- c(accuracy, trainr_data[[2]])
}

results$accuracy <- accuracy
ct_county_acc19 <- mean(results$accuracy, na.rm=T)
```
...
kable(data.frame("Model Type"=c("Overall Linear Regression", "Overall Random Forest", "Overall Classification Tree",
                 ...
                 "Test Accuracy 2019"=c(lm_overall_acc19, rf_overall_acc19, ct_overall_acc19, 
                                        lm_county_acc19, rf_county_acc19, ct_county_acc19),
                "Test Accuracy 2020"=c(lm_overall_acc20, rf_overall_acc20, ct_overall_acc20, 
                                       lm_county_acc20, rf_county_acc20, ct_county_acc20)))
```
